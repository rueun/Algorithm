...
public class P2178_ex27 {
	// 상하좌우를 탐색하기 위한 배열 선언하기
	static int[] dx = {0,1,0,-1}; // 왼쪽 : 1, 오른쪽 : -1
	static int[] dy = {1,0,-1,0}; // 위 : 1, 아래 : -1
	...
	public static void main(String[] args) throws IOException {
		...
		StringTokenizer st = new StringTokenizer(br.readLine()); // N : 행, M : 열
		...
		while(!queue.isEmpty()) {
			...
			for(int k=0; k<4; k++) { // 상하좌우 탐색
				...
				if(x>=0 && y>=0 && x<N && y<M) { // 좌표 유효성 검사하기
					if(A[x][y]!=0 && !visited[x][y]) { // 갈 수 있는 칸 && 방문검사하기
